...
function calcularIMC() {
    ...
    if (!nome || !serie || isNaN(idade) || !sexo || isNaN(peso) || isNaN(altura) || altura <= 0) {
    ...
    const classificacao = idade >= 6 && idade <= 12
        ? classificarIMCInfantil(imc, idade, sexo)
        : classificarIMCAdulto(imc);

    const sexoDescricao = sexo === "M" ? "Masculino" : "Feminino";
    ...
    resultadoDiv.innerHTML = `
        ...
        <p><strong>Sexo:</strong> ${sexoDescricao}</p>
        <p><strong>IMC:</strong> ${imcFormatado}</p>
        <p><strong>Classificação:</strong> ${classificacao}</p>
        <p style="font-size:12px;color:#666;">
            Classificação baseada em referência por idade (modelo educacional).
        </p>
    `;
...
function classificarIMCInfantil(imc, idade, sexo) {
    ...
    if (!referencia) {
        return "Faixa etária fora da tabela infantil";
    }

    if (imc < referencia.p5) {
        return "Baixo peso (Percentual < 5%)";
    } else if (imc < referencia.p85) {
        return "Peso adequado (Percentual entre 5% e 84%)";
    } else if (imc < referencia.p95) {
        return "Sobrepeso (Percentual entre 85% e 94%)";
    } else {
        return "Obesidade (Percentual ≥ 95%)";
    }
}
